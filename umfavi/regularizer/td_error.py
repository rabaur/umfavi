...
def td_error_regularizer(
    ...
    q_curr = torch.gather(q_values, dim=-1, index=acts_curr).squeeze(-1)  # (batch_size, ..., num_steps, 1)
    
    # Select Q(s_{t+1}, a_{t+1}) for next state-action pairs
    q_next = torch.gather(q_values, dim=-1, index=acts_next).squeeze(-1)  # (batch_size, ..., num_steps - 1, 1)

    # Compute TD-error (which should equal the reward)
    td_error = q_curr - gamma[0].item() * q_next  # (batch_size, num_steps)
    print(f"last q value estimates: {q_values[..., -1, :]}")
    print(f"last td_error: {td_error[..., -1, :]}")
    ...
    valid_mask = ~dones[..., :-1, :].squeeze(-1).bool()  # True for valid timesteps
